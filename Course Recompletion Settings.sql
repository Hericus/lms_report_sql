SELECT
    ...
    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'recompletiontype') = 'period' THEN 'Yes'
        ELSE 'No'
    END AS "Enabled",
    ...
    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'quiz') = '2' THEN 'Give student extra attempts'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'quiz') = '1' THEN 'Delete_existing attempts'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'quiz') = '0' THEN 'Do nothing'
        ELSE NULL
    END AS "Quiz attempts",

    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'archivequiz') = '1' THEN 'Yes'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'archivequiz') = '0' THEN 'No'
        ELSE NULL
    END AS "Archive quiz attempts",

    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'scorm') = '1' THEN 'Delete_existing attempts'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'scorm') = '0' THEN 'Do nothing'
    END AS "SCORM attempts",

    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'archivescorm') = '1' THEN 'Yes'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'archivescorm') = '0' THEN 'No'
        ELSE NULL
    ...
    CASE
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'assign') = '1' THEN 'Delete_existing attempts'
        WHEN (SELECT rc1.value FROM prefix_local_recompletion_config AS rc1 WHERE rc1.course = c.id AND rc1.name = 'assign') = '0' THEN 'Do nothing'
        ELSE NULL
...
FROM prefix_local_recompletion_config AS rc
        ...
        LEFT JOIN prefix_local_recompletion_config AS enabled ON enabled.course = c.id AND enabled.name = 'recompletiontype'
        LEFT JOIN prefix_local_recompletion_config AS archive ON archive.course = c.id AND archive.name = 'archivecompletiondata'
WHERE 1=1
    %%FILTER_SUBCATEGORIES:cc.path%%
    %%FILTER_COURSES:c.id%%
    %%FILTER_SQL_enabled:enabled.value:=%%
    %%FILTER_SQL_archive:archive.value:=%%
GROUP BY
    c.id
