SELECT
    ...
    to_char(to_timestamp(p.latestcomp),'YYYY-MM-DD') "Date completed",
...
FROM
    prefix_local_recompletion_cc_cached AS p
        JOIN prefix_course AS c ON p.courseid = c.id
...
%%FILTER_STARTTIME:p.latestcomp:>=%%
%%FILTER_ENDTIME:p.latestcomp:<%%
